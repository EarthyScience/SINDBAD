function [f,fe,fx,s,d,p] = prec_wSoilBase_smax1Layer(f,fe,fx,s,d,p,info)
% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% defines the maximum soil water content of 1 soil layer as fraction of
% the soil depth defined in the ModelStructure.json
% based on the TWS model for the Northern Hemisphere 
%
% Inputs:
...
if numel(soilDepths)                ~=  nSoilLayers &&  numel(soilDepths) ~= 2
    error(['prec_wSoilBase_smax1Layer: the number of soil layers in modelStructure.json does not match with soil depths specified. This approach needs 1 soil layer.'])
end
...
% 1st layer
s.wd.p_wSoilBase_wSat(:,1)          =   p.wSoilBase.smax .* soilDepths(1);
s.wd.p_wSoilBase_wFC(:,1)           =   p.wSoilBase.smax .* soilDepths(1);
