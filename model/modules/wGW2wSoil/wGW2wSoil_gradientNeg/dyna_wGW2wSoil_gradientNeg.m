function [f,fe,fx,s,d,p] = dyna_wGW2wSoil_gradientNeg(f,fe,fx,s,d,p,info,tix)
% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% calculates a buffer storage that doesn't give water to the soil when the soil dries up, while the soil gives water to the wGW when the soil is
...
tmp  = min(fe.wGW2wSoil.potFlux(:,tix), min(s.w.wGW, s.wd.p_wSoilBase_wSat(:,wSoilend) - s.w.wSoil(:,wSoilend)));
tmp  = max(tmp, max(-s.w.wSoil(:,wSoilend), -(s.wd.p_wGW2wSoil_gwmax - s.w.wGW))); % use here the fx.GW2Soil from above! 
