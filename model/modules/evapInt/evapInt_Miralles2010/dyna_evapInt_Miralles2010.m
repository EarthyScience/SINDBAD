...
%   - 1.1 on 22.11.2019 (skoirala): handle s.cd.fAPAR
%
% NOTES: 
...
Pgc(valids)     =   -1.*( rainInt(valids) .* CanopyStorage(valids) ./...
                    ((1- fte(valids) ) .* evapRate(valids) )).*log(1-((1- fte(valids) ) .* evapRate(valids) ./ rainInt(valids) ));

%Pgt: amount of gross rainfall necessary to saturate the trunks
Pgt(valids)     =   Pgc(valids) + rainInt(valids) .* St(valids) ./...
                    (pd(valids) .* s.cd.fAPAR(valids) .* ( rainInt(valids) - evapRate(valids) .* (1 - fte(valids) )));
