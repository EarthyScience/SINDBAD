...
for sl=wSoilend:-1:2
    dosSoilLower                  =  s.w.wSoil(:,sl) ./ p.pSoil.smax2;
    k_unsatfrac_soil_lower        =  min((dosSoilLower) .^ (2.*p.pSoil.chb + 3),1);
    k_unsat_lower                 =   p.pSoil.kSat .* k_unsatfrac_soil_lower;
    
    dosSoilUpper                  =  s.w.wSoil(:,sl-1) ./ p.pSoil.smax1;
    k_unsatfrac_soil_upper        =  min((dosSoilUpper) .^ (2.*p.pSoil.chb + 3),1);
    k_unsat_upper                 =   p.pSoil.kSat .* k_unsatfrac_soil_upper;
