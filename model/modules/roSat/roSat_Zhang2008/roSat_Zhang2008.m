...
% Inputs:
%	- f.PET: potential ET
%   - s.wd.p_wSoilBase_wAWC: maximum available water in soil per soil layer
...
valids          =   s.wd.WBP > 0;
%--> set roSat 
roSat(valids)    =   s.wd.WBP(valids) - s.wd.WBP(valids) .*(1 + X0(valids) ./s.wd.WBP(valids) - ( 1 + (X0(valids) ./s.wd.WBP(valids)).^(1./ p.roSat.alpha(valids) ) ).^ p.roSat.alpha(valids) ); 
fx.roSat(:,tix)  =   roSat;
