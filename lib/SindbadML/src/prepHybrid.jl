...
function getIndicesSplit(info, site_indices, ::CalcFoldFromSplit)
    ...
    @show train_ratio, val_ratio, test_ratio
...
function prepHybrid(forcing, observations, info, ::MLTrainingType)
    ...
    @info "  Building loss function handles for every site"
    loss_functions, loss_component_functions = getLossFunctionHandles(info, run_helpers, sites_forcing)

    ## split the sites

    @info "  Getting indices and sites for training, validation and testing"
    ...
    @info "    Total sites: $(length(sites_forcing))"
    @info "    Training sites: $(length(sites.training))"
    @info "    Validation sites: $(length(sites.validation))"
    @info "    Testing sites: $(length(sites.testing))"

    ## get covariates

    @info "  Loading Covariates for $(info.hybrid.covariates.variables) from $(info.hybrid.covariates.path)"
    xfeatures = loadCovariates(sites_forcing; kind=info.hybrid.covariates.variables, cube_path=info.hybrid.covariates.path)
    @info "    Min/Max of features: [$(minimum(xfeatures)), $(maximum(xfeatures))]"
    ...
    @info "    Method: $(nameof(typeof(info.hybrid.ml_optimizer.method)))"
