...
function nn_model(n_features, neurons, n_out; seed = 123)
    Random.seed!(seed)
    model = Flux.Chain(
        Flux.Dense(n_features, neurons, Flux.relu),
        Flux.Dense(neurons, n_out) 
    )
    return model
end
...
"""
function machine(trainloader, target_param, loss, sloss, nn_mod;
    nepochs=100, opt=Optimisers.Adam(), is_logging=false, rseed=534)
    ...
    for epoch in 1:nepochs
        ...
        opt_param = [nn_mod(s[1]) for s in trainloader]
