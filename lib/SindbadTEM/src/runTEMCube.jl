...
function coreTEMYax(selected_models, loc_forcing, loc_land, tem_info)
    ...
    land_prec = precomputeTEM(selected_models, loc_forcing_t, land_prec, tem_info.model_helpers)    
    land_spin = spinupTEM(selected_models, spinup_forcing, loc_forcing_t, land_prec, tem_info) # ? not good? see git diff
    # land_spin = land_prec
...
"""
    run_psTEMYax(selected_models::Tuple, forcing::NamedTuple, in_cube_params, tbl_params, info::NamedTuple)
