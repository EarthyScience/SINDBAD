...
function runTEMCore(
    ...
    land_stacked = runTimeLoop(selected_models,
        forcing,
        land_spin,
        f_one,
        tem_helpers,
...
function TEM(forcing::NamedTuple, info::NamedTuple)
    ...
    land_time_series = runTEMCore(selected_models, loc_forcings[1], tem_with_vals.helpers, tem_with_vals.models, tem_with_vals.spinup, land_init_space[1], f_one, tem_with_vals.helpers.run.spinup.run_spinup)
...
function TEM(selected_models::Tuple,
    ...
    f_one)
    land_time_series = runTEMCore(selected_models, forcing, tem_with_vals.helpers, tem_with_vals.models, tem_with_vals.spinup, land_init, f_one, tem_with_vals.helpers.run.spinup.run_spinup)
    return landWrapper(land_time_series)
end