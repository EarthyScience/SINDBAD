...
function runExperiment(info::NamedTuple, forcing::NamedTuple, ::DoCalcCost)
    ...
    loss_vector = lossVector(forward_output, obs_array, cost_options)
...
function runExperimentOpti(sindbad_experiment::String; replace_info=Dict())
    ...
    loss_vector = lossVector(fp_output.output.optimized, opti_output.observation, cost_options)
    loss_vector_def = lossVector(fp_output.output.default, opti_output.observation, cost_options)
