...
%             paramRem        = strfind(paramInput,module); % did not work before if it was a pasrtial match...
            ...
            if exist(file_json,'file')
                ...
                disp(['MSG : readConfigFiles : no parameter config file (json) existing for module : ' module ' : approach: ' appr]);
            end
    ...
    if info.tem.model.flags.runOpti
        ...
        if idxError ~= 0
            for ii=1:length(idxError)
                warning(['Optimization parameter: ' info.opti.paramsList{idxError(ii)} ' not represented in the model structure and excluded from optimization!']);
