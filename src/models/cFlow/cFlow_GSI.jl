...
function compute(o::cFlow_GSI, forcing, land, infotem)
	...
	f_now = (1.0 - f_τ) * f_prev + f_τ * f_tmp
	fWfTfR = f_now
	...
	LR2Re = min(max(-slope_fWfTfR, 0) * LR2ReSlp, 1); # * (cVeg_growth < 0.0)
	Re2LR = min(max(slope_fWfTfR, 0) * Re2LRSlp, 1); # * (cVeg_growth > 0.0)
	KShed = min(max(-slope_fWfTfR, 0) * kShed, 1)
	...
	p_k[cVegLeafzix] = min((p_k[cVegLeafzix] + k_Lshed + L2Re), 1)
	L2ReF = L2Re / (p_k[cVegLeafzix])
	k_LshedF = k_Lshed / (p_k[cVegLeafzix])
	p_k[cVegRootzix] = min((p_k[cVegRootzix] + k_Rshed + R2Re), 1)
	R2ReF = R2Re / (p_k[cVegRootzix])
	k_RshedF = k_Rshed / (p_k[cVegRootzix])
	p_k[cVegReservezix] = min((p_k[cVegReservezix] + Re2L + Re2R), 1)
...
function update(o::cFlow_GSI, forcing, land, infotem)
	# @unpack_cFlow_GSI o
	return land
end
...
# update
update pools and states in cFlow_GSI
 - land.cFlow.p_A
