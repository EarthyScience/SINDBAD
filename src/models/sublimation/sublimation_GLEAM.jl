export sublimation_GLEAM, sublimation_GLEAM_h
"""
...
function precompute(o::sublimation_GLEAM, forcing, land, infotem)
...
function compute(o::sublimation_GLEAM, forcing, land, infotem)
	...
	# Murphy & Koop [2005] here attached.
	# The slope of the vapour pressure over ice versus temperature curve
	# (Δ) is obtained from eq. (7). You may want to do this derivative
	...
	# Then the psychrometer "constant" (γ) can be calculated in [kPa/K]
	# according to Brunt [1952] as:
	# Where P is the air pressure in [kPa], which I consider as a function of
	# the elevation [DEM] but can otherwise be set to 101.3, & ca is the
...
# precompute:
precompute/instantiate time-invariant variables for sublimation_GLEAM
...
*Created by:*
 - Martin Jung [mjung]
"""
function sublimation_GLEAM_h end