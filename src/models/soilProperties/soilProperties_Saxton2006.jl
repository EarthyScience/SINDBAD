export soilProperties_Saxton2006, kSaxton2006, soilParamsSaxton2006, soilProperties_Saxton2006_h
"""
...
function update(o::soilProperties_Saxton2006, forcing, land, infotem)
	# @unpack_soilProperties_Saxton2006 o
	return land
end
...
# update
update pools and states in soilProperties_Saxton2006
 - None
...
"""
function soilProperties_Saxton2006_h end
...
function kSaxton2006(land, infotem, sl)
	@unpack_land begin
		(nLookup, p_kLookUp, p_kPow, p_logkSat, p_wSat) ∈ land.soilWBase
		soilW ∈ land.pools
	end
	...
	if makeLookup
		...
		# takes 1.370 seconds
		kPow = p_kPow[sl]
		logkSat = p_logkSat[sl]
		K = exp(kPow * log(θ_dos) + logkSat)
	else
		θ_dos[θ_dos < 0.0] = 0.0
		θ_dos[imag(θ_dos)!= 0] = 0.0
		lkDat = p_kLookUp[sl]
		lkInd = fix(θ_dos * nLookup)
		lkInd[lkInd == 0] = 1
...
function soilParamsSaxton2006(land, infotem, sl)
	@unpack_land begin
		(p_CLAY, p_ORGM, p_SAND) ∈ land.soilTexture
	end
