...
function setPoolsInfo(info::NamedTuple)
    ...
    tmp_states = (;)
    hlp_states = (;)
    model_array_type = getfield(SindbadCore, toUpperCaseFirst(info.settings.experiment.exe_rules.model_array_type, "ModelArray"))()
    @show model_array_type
...
function createInitPools(info_pools::NamedTuple, tem_helpers::NamedTuple)
    ...
    for element ∈ propertynames(info_pools)
        ...
        model_array_type = getfield(SindbadCore, toUpperCaseFirst(string(getfield(props, :arraytype)), "ModelArray"))()
