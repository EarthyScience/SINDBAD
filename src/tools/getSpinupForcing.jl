...
function extracttimestep(forcing, tstart, tend)
    map(forcing) do v
...
"""
setSpinupForcing(forcing, info, ::Val{:full})
...
function setSpinupForcing(forcing, info, ::Val{:recycleMSC})
    # spinup_forcing = extracttimestep(forcing, 1, 365)
    spinup_forcing = forcing[1:365]
    return spinup_forcing
end

"""
setSpinupForcing(forcing, info, ::Val{:mean})
