            ...
            if subkey == last(key_list)
                ...
                if !haskey(key_dict, subkey_name)
                    ...
                    key_dict[subkey_name][key_list[key_index+1]] =
                        key_dict[key_list[key_index+1]*string(key_index + 1)]
                else
                    tmp = Dict()
...
function setupOutputDirectory(infoTuple::NamedTuple)
    ...
    if isnothing(outpath)
        ...
        out_path_new =
            joinpath(join(split(infoTuple.settings_root, "/")[1:end-1], "/"), out_path_new)
    ...
    out_path_new =
        out_path_new * infoTuple.experiment.domain * "_" * infoTuple.experiment.name
...
function getConfiguration(sindbad_experiment::String; replace_info = nothing)
    ...
    if endswith(sindbad_experiment, ".json")
        info_exp =
            getExperimentConfiguration(sindbad_experiment; replace_info = replace_info)
