...
function setPoolsInfo(info::NamedTuple)
    ...
    tmp_states = (;)
    hlp_states = (;)
    model_array_type = getfield(SetupSimulation, toUpperCaseFirst(info.settings.experiment.exe_rules.model_array_type, "ModelArray"))()
...
function createInitPools(info_pools::NamedTuple, tem_helpers::NamedTuple)
    ...
    for element ∈ propertynames(info_pools)
        ...
        model_array_type = getfield(SetupSimulation, toUpperCaseFirst(string(getfield(props, :arraytype)), "ModelArray"))()
