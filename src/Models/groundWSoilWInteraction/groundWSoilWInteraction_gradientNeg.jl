...
function compute(o::groundWSoilWInteraction_gradientNeg, forcing, land, helpers)
    ...
    tmp_gradient =
        sum(groundW + ΔgroundW) / p_gwmax - (soilW[end] + ΔsoilW[end]) / p_wSat[end] # the sign of the gradient gives direction of flow: positive = flux to soil; negative = flux to gw from soilW
