...
#! format: off
@bounds @describe @units @with_kw struct autoRespiration_Thornley2000C{T1,T2,T3} <: autoRespiration
...
function define(p_struct::autoRespiration_Thornley2000C, forcing, land, helpers)
    @unpack_land begin
        cEco ∈ land.pools
    end
    ...
    ## pack land variables
    @pack_land begin
        (p_km, p_km4su, Fd) => land.autoRespiration
        (auto_respiration_growth, auto_respiration_maintain, c_eco_efflux) => land.states
    end
    return land
end

function compute(p_struct::autoRespiration_Thornley2000C, forcing, land, helpers)
    ## unpack parameters
    @unpack_autoRespiration_Thornley2000C p_struct
    ...
    for ix ∈ zix
        ...
        km_ix = min_1(o_one / p_C2Nveg[ix] * RMN * auto_respiration_f_airT)
        kd_ix = Fd[ix]
        p_km_ix = km_ix * kd_ix
        p_km4su_ix = p_km[ix] * (o_one - YG)

        # maintenance respiration: R_m = km * (1.0 - YG) * C; km = km * MTF [before equivalent to kd]
        RA_M_ix = p_km_ix * (o_one - YG) * cEco[ix]
