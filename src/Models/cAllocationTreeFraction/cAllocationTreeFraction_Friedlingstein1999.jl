...
function instantiate(o::cAllocationTreeFraction_Friedlingstein1999, forcing, land, helpers)
    ## unpack parameters
    ## calculate variables
...
function compute(o::cAllocationTreeFraction_Friedlingstein1999, forcing, land, helpers)
    ...
    for ix in getzix(land.pools.cVegRoot, helpers.pools.zix.cVegRoot)
        r0 = r0 + cAlloc[ix]
    end
    s0 = zero(eltype(cAlloc)) 
    for ix in getzix(land.pools.cVegWood, helpers.pools.zix.cVegWood)
        s0 = s0 + cAlloc[ix]
    end
    l0 = zero(eltype(cAlloc)) 
    for ix in getzix(land.pools.cVegLeaf, helpers.pools.zix.cVegLeaf)
        l0 = l0 + cAlloc[ix]
