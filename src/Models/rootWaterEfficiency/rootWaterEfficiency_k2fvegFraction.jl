...
function compute(p_struct::rootWaterEfficiency_k2fvegFraction, forcing, land, helpers)
    ...
    k1_root_water_efficiency = min1(frac_vegetation * k1_scale) # the fraction of water that a root can uptake from the 1st soil layer
    k2_root_water_efficiency = min1(frac_vegetation * k2_scale) # the fraction of water that a root can uptake from the 1st soil layer
    # set the properties
    # 1st Layer
