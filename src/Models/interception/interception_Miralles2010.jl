...
@bounds @describe @units @with_kw struct interception_Miralles2010{T1, T2, T3, T4, T5} <: interception
	CanopyStorage::T1 = 1.2f0 | (0.4f0, 2.0f0) | "Canopy storage" | "mm"
	fte::T2 = 0.02f0| (0.02f0, 0.02f0) | "fraction of trunk evaporation" | ""
	evapRate::T3 = 0.3f0 | (0.1f0, 0.5f0) | "mean evaporation rate" | "mm/hr"
	St::T4 = 0.02f0 | (0.02f0, 0.02f0) | "trunk capacity" | "mm"
	pd::T5 = 0.02f0 | (0.02f0, 0.02f0) | "fraction rain to trunks" | ""
end
...
function compute(o::interception_Miralles2010, forcing, land::NamedTuple, helpers::NamedTuple)
	...
	valids = rainInt > 0.0f0 & fAPAR > 0.0f0;
	Pgc = 0.0f0
	Pgt = 0.0f0
	Ic = 0.0f0
	Ic1 = 0.0f0
	...
	Pgc = -1 * (rainInt * CanopyStorage / ((1.0f0 - fte) * evapRate)) * log(1.0f0 - ((1.0f0 - fte) * evapRate / rainInt))
	#Pgt: amount of gross rainfall necessary to saturate the trunks
	Pgt = Pgc + rainInt * St / (pd * fAPAR * (rainInt - evapRate * (1.0f0 - fte)))
	#Ic: interception loss from canopy
	Ic1 = fAPAR * rain; #Pg < Pgc
	Ic2 = fAPAR * (Pgc+((1.0f0 - fte) * evapRate / rainInt) * (rain - Pgc)); #Pg > Pgc
	v = rain <= Pgc & valids == 1
	Ic[v] = Ic1[v]
	Ic[v == 0] = Ic2[v == 0]
	#It: interception loss from trunks
	#It1 = St;# Pg < Pgt
	It2 = pd * fAPAR * (1.0f0 - (1.0f0 - fte) * evapRate / rainInt) * (rain - Pgc);#Pg > Pgt
	v = rain <= Pgt
	It[v] = St[v]
	It[v == 0] = It2[v == 0]
	tmp = Ic+It
	tmp[rain == 0.0f0] = 0.0f0
	v = tmp > rain
	tmp[v] = rain[v]
	interception = tmp
