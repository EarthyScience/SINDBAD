...
function yx_ecoLoc(approaches::Tuple, forcing::NamedTuple, tem::NamedTuple, outcubes, additionaldims, loc_names)
    # @show forcing
    ...
    # @show loc_output
    yx_coreEcosystem(approaches, loc_forcing, tem, loc_output)
end
...
@noinline function yx_theRealtimeLoopForward(forward_models::Tuple, forcing::NamedTuple, out::NamedTuple, tem_variables::NamedTuple, tem_helpers::NamedTuple, time_steps, loc_output, otype, oforc)
    map(1:time_steps) do ts
        ...
        yx_runModels(f, forward_models, out, tem_helpers)::otype
...
function yx_coreEcosystem(approaches, loc_forcing, tem, loc_output)
...
"""
function yx_runEcosystem(approaches::Tuple, forcing::NamedTuple, tem::NamedTuple, outcubes)
    ...
    qbmap(Iterators.product(Base.OneTo.(spacesize)...)) do loc_names
        yx_ecoLoc(approaches, forcing, tem, outcubes,additionaldims, loc_names)
    end
end
...
#         spacesize = values(tem.helpers.run.loop[additionaldims])
#         qbmap(Iterators.product(Base.OneTo.(spacesize)...)) do loc_names
#             return yx_ecoLoc(approaches, forcing, tem, outcubes,additionaldims, loc_names)
#         end
#     else
#         yx_coreEcosystem(approaches, forcing, tem, outcubes)
#     end
# end
...
function yx_doRunEcosystem(args...; tem::NamedTuple, forward_models::Tuple, forcing_variables::AbstractArray)
    ...
    yx_runEcosystem(forward_models, forcing, tem, outputs)
    outputs
end


function yx_mapRunEcosystem(forcing::NamedTuple, output::NamedTuple, tem::NamedTuple, forward_models::Tuple; max_cache=1e9)
