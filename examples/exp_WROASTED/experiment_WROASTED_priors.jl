...
develop_f = () -> begin
    ...
    loc_space_maps, loc_space_names, loc_space_inds, loc_forcings, loc_outputs, land_init_space, f_one, loc_forcing, loc_output  = prepRunEcosystem(output, tem.models.forward, forcing, tem);
    ...
    m_sesamfit = Turing.@model function sesamfit(obs, ::Type{T} = Float64) where {T}
        ...
        runEcosystem!(output.data, output.land_init, newApproaches, forcing, tem, loc_space_maps, loc_space_names, loc_space_inds, loc_forcings, loc_outputs, land_init_space, f_one)
