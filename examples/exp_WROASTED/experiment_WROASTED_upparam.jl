...
opt = rand(length(info.optim.optimized_parameters));
ori = info.tem.models.forward;
tblParams.optim .= opt;
new = updateParameters(tblParams, ori, opt);
new3 = updateParameters(tblParams, ori, opt);

"""
updateParameters(tblParams, approaches)
"""
function updateParameters(tblParams, approaches::Tuple, popt)
    function filtervar(var, modelName, tblParams, approachx, poptv)
    ...
    for (idx, modelName) in enumerate(namesApproaches)
        ...
        newapproachx = if modelName in tblParams.modelsApproach
            ...
            for v in eachindex(vars)
                var = vars[v]
                inOptim = filtervar(var, modelName, tblParams, approachx, popt[v])
