...
transpirationKA = KeyedArray(transpiration_synt .|> Float32; time = obs.gpp.time, site = obs.gpp.site)

evapotranspirationOut = output.data[4]
evapotranspiration_synt = reshape(evapotranspirationOut, (4748, 205));
evapotranspirationKA = KeyedArray(evapotranspiration_synt .|> Float32; time = obs.gpp.time, site = obs.gpp.site)


obs_synt = (;
    gpp = gppKA, gpp_σ = obs.gpp_σ, gpp_mask = obs.gpp_mask,
    nee = neeKA, nee_σ = obs.nee_σ, nee_mask = obs.nee_mask,
    transpiration = transpirationKA, transpiration_σ = obs.transpiration_σ, transpiration_mask = obs.transpiration_mask,
    evapotranspiration = evapotranspirationKA, evapotranspiration_σ = obs.evapotranspiration_σ, evapotranspiration_mask = obs.evapotranspiration_mask
    )
