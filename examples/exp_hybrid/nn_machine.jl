...
function shuffle_indxs(sites, n_bs_sites, mb_idxs; seed=1)
    ...
    xbatches = [s_sites[p] for p in mb_idxs if length(p)==n_bs_sites]
    return xbatches
end
...
function nn_machine(nn_args, x_args, xfeatures, grads_args; nepochs=10)
    ...
    for epoch in 1:nepochs
        ...
        xbatches = x_args.shuffle ? shuffle_indxs(x_args.sites, x_args.bs, mb_idxs; seed=epoch) : xbatches
        for (batch_id, xbatch) in enumerate(xbatches)
            ∇params = get_∇params(xfeatures, re, flat, xbatch, nn_args.n_params, x_args.bs, grads_args)
