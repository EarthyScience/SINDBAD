...
for (i, tj) in enumerate(tjs)
     ...
     loc_space_maps, loc_space_names, loc_space_inds, loc_forcings, loc_outputs, land_init_space, f_one = prepRunEcosystem(output.data, land_init_space[1], info.tem.models.forward, forc, forcing.sizes, info.tem);
...
for (i, cp) in enumerate(cpnames)
    p = plot(expSol[i, :], odeSol[i, :], lw=0, marker=:o, size=(600, 900));
    title!("$(i): $(string(cp))");
    x_lims = min.(minimum(expSol[i, :]), minimum(odeSol[i, :])), max.(maximum(expSol[i, :]), maximum(odeSol[i, :]));
    xlims!(x_lims);
