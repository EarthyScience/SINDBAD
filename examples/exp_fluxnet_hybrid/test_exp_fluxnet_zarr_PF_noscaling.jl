...
foreach(costOpt) do var_row
    ...
    plot!(xdata, opt_var; color=:seagreen3, label="opt ($(round(metr_opt, digits=2)))", lw=1.5, ls=:dash, color=:seagreen3)
...
for (o, v) in enumerate(output_vars)
    ...
    if size(opt_var, 2) == 1
        ...
        plot!(xdata, opt_var, color=:seagreen3[:, 1]; label="opt ($(round(SindbadTEM.mean(opt_var[:, 1]), digits=2)))", color=:seagreen3)
        ...
        foreach(axes(opt_var, 2)) do ll
            ...
            plot!(xdata, opt_var[:, ll]; color=:seagreen3, label="opt ($(round(SindbadTEM.mean(opt_var[:, ll]), digits=2)))", color=:seagreen3)
            ylabel!("$(vinfo["standard_name"])", font=(20, :green))
