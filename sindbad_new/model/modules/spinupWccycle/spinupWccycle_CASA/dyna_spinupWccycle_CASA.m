function [fx,s,d] = dyna_spinupWccycle_CASA(f,fe,fx,s,d,p,info,tix)
% #########################################################################
% FUNCTION	: dyna_spinupWccycle_CASA
% 
% PURPOSE	: 
% 
% REFERENCES:
...
% s.cPools(ii).value
% fe.spinupWccycle.DecayRate(ii).value
...
MTF     = fe.spinupWccycle.MTF;
BGME	= d.rhFwsoil.BGME(:,tix);

% ALLOCATION NPP TO pvegRETATION POOLS
for ii = 1:4
...
% CARBON POOLS
for ii = 1:4
    POTcOUT(:,ii)  = min(s.cPools(ii).value,s.cPools(ii).value .* fe.spinupWccycle.DecayRate(ii).value(:,tix));
...
for ii = 5:14
    POTcOUT(:,ii)   = min(s.cPools(ii).value, s.cPools(ii).value .* fe.spinupWccycle.kfEnvTs(ii).value(:,tix) .* BGME);
    % make sure the soil respiratory fluxes are 0
	fx.cEfflux(ii).value(:,tix)	= 0;
end

% COMPUTE CARBON FLUXES IN THE psoilR
...
for ij = 1:numel(flux_order)
    ...
    idonor                          = fe.spinupWccycle.ctransfer(ii).donor;
    ireceiver                       = fe.spinupWccycle.ctransfer(ii).receiver;
    cOUT                            = POTcOUT(:,idonor) .* fe.spinupWccycle.ctransfer(ii).xtrEFF;
    s.cPools(idonor).value          = s.cPools(idonor).value    - cOUT;
    s.cPools(ireceiver).value       = s.cPools(ireceiver).value + cOUT .* fe.spinupWccycle.ctransfer(ii).effFLUX;
    fx.cEfflux(idonor).value(:,tix)   = fx.cEfflux(idonor).value(:,tix)  + cOUT .* (1 - fe.spinupWccycle.ctransfer(ii).effFLUX);
end
