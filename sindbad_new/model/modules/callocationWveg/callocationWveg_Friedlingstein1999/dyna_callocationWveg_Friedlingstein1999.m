function [fx,s,d] = dyna_callocationWveg_Friedlingstein1999(f,fe,fx,s,d,p,info,tix)
% #########################################################################
% FUNCTION	: dyna_callocationWveg_Friedlingstein1999
% 
...
% constants
so      = p.callocationWveg.so;
minL    = p.callocationWveg.minL;
maxL    = p.callocationWveg.maxL;
minL_fW = p.callocationWveg.minL_fW;
maxL_fW = p.callocationWveg.maxL_fW;
RelY    = p.callocationWveg.RelY;

% precomputations
NL_fT	= d.callocationWveg.NL_fT(:,tix);
...
WL              = s.wSM ./ d.callocationWveg.WLDenominator;
...
d.callocationWveg.cf2Root(:,tix)	= d.callocationWveg.RootNumerator(:,tix) ./ (d.callocationWveg.LL(:,tix) + RelY .* minWLNL);
d.callocationWveg.cf2Wood(:,tix)	= so .* (RelY + 1) .* minWLNL ./ (RelY .* d.callocationWveg.LL(:,tix) + minWLNL);
d.callocationWveg.cf2Leaf(:,tix)	= 1 - d.callocationWveg.cf2Root(:,tix) - d.callocationWveg.cf2Wood(:,tix);

% adjust allocation
d	= calcAdjAllocation(f,fe,fx,s,d,p,info,tix);

% check allocation
checkcallocationWveg(f,fe,fx,s,d,p,info,tix);
