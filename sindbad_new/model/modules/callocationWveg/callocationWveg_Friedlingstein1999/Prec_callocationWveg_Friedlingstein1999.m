function [fe,fx,d,p] = prec_callocationWveg_Friedlingstein1999(f,fe,fx,s,d,p,info)
% #########################################################################
% FUNCTION	: prec_callocationWveg_Friedlingstein1999
% 
...
% Check callocationWveg_Friedlingstein1999 for details.
% 
% INPUT
...
% constants
ro      = p.callocationWveg.ro;
kext	= p.callocationWveg.kext;
minL    = p.callocationWveg.minL;
maxL    = p.callocationWveg.maxL;
minL_fT = p.callocationWveg.minL_fT;
maxL_fT = p.callocationWveg.maxL_fT;
...
% send it to d
d.callocationWveg.NL_fT	= NL_fT;

% start computation of pseudo NL
NL                      = minL.*ones(size(f.PET));
% send it to d
d.callocationWveg.NL	= NL;

% numerator of the root allocation function
d.callocationWveg.RootNumerator	= ro .* (RelY + 1) .* LL;

% denominator of the water limitation fuction
d.callocationWveg.WLDenominator	= p.psoilR.tAWC;
