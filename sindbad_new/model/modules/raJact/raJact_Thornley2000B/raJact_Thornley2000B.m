function [fx,s,d] = raJact_Thornley2000B(f,fe,fx,s,d,p,info,i)
% #########################################################################
% FUNCTION	: raJact_Thornley2000B
% 
...
for ii = 1:4
    ...
    fx.cEfflux(ii).growth(:,i)	= (1 - p.AutoResp.YG) .* fx.gpp(:,i) .* d.CAllocationVeg.c2pool(ii).value(:,i);
    
    % maintenance respiration: R_m = km * (C + YG * GPP * allocationToPool)
    fx.cEfflux(ii).maintenance(:,i)	= fe.AutoResp.km(ii).value(:,i) .* (s.cPools(ii).value + p.AutoResp.YG .* fx.gpp(:,i) .* d.CAllocationVeg.c2pool(ii).value(:,i));
