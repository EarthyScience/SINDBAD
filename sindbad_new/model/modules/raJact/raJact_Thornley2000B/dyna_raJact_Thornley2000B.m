function [fx,s,d] = dyna_raJact_Thornley2000B(f,fe,fx,s,d,p,info,tix)
% #########################################################################
% FUNCTION	: dyna_raJact_Thornley2000B
% 
% PURPOSE	: estimate autotrophic respiration as maintenance + growth
% respiration according to Thornley and Cannell (2000): MODEL B -
% growth respiration is given priority (check Fig.1 of the paper).
% 
% REFERENCES:
...
% 
% Questions - practical - leave raJact per pool, or make a field fx.ra
...
for ii = 1:4
    ...
    fx.cEfflux(ii).growth(:,tix)	= (1 - p.raJact.YG) .* fx.gpp(:,tix) .* d.callocationWveg.c2pool(ii).value(:,tix);
    
    % maintenance respiration: R_m = km * (C + YG * GPP * allocationToPool)
    fx.cEfflux(ii).maintenance(:,tix)	= fe.raJact.km(ii).value(:,tix) .* (s.cPools(ii).value + p.raJact.YG .* fx.gpp(:,tix) .* d.callocationWveg.c2pool(ii).value(:,tix));
    
end

% compute total respiration and npp for each vegetation pool
[fx,s,d]	= calcUpdNPPRa(f,fe,fx,s,d,p,info,tix);
