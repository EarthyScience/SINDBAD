function [fx,s,d] = dyna_raJact_Thornley2000A(f,fe,fx,s,d,p,info,tix)
% #########################################################################
% FUNCTION	: dyna_raJact_Thornley2000A
% 
...
for ii = 1:4
    ...
    fx.cEfflux(ii).maintenance(:,tix)	= fe.raJact.km(ii).value(:,tix) .* s.cPools(ii).value;
    
    % growth respiration: R_g = (1 - YG) * (GPP * allocationToPool - R_m)
    fx.cEfflux(ii).growth(:,tix)	= (1 - p.raJact.YG) .* (fx.gpp(:,tix) .* d.callocationWveg.c2pool(ii).value(:,tix) - fx.cEfflux(ii).maintenance(:,tix));
    
    % no negative growth respiration
    fx.cEfflux(ii).growth(fx.cEfflux(ii).growth(:,tix) < 0,tix)	= 0;
