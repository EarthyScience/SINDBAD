function [fx,s,d,f] = dyna_RAact_Thornley2000A(f,fe,fx,s,d,p,info,tix)
% #########################################################################
...
for zix = info.tem.model.variables.states.c.cVeg.zix
    ...
    fx.cEfflux(zix).maintenance(:,tix)	= fe.RAact.km(zix).value(:,tix) .* s.c.cEco(:,zix);
    
    % growth respiration: R_g = (1 - YG) * (GPP * allocationToPool - R_m)
    fx.cEfflux(zix).growth(:,tix)	= (1 - p.RAact.YG) .* (fx.gpp(:,tix) .* d.cAlloc.c2pool(zix).value(:,tix) - fx.cEfflux(zix).maintenance(:,tix));
    
    % no negative growth respiration
    fx.cEfflux(zix).growth(fx.cEfflux(zix).growth(:,tix) < 0,tix)	= 0;
