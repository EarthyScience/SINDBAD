function [fx,s,d] = dyna_RAact_Thornley2000A(f,fe,fx,s,d,p,info,tix)
% #########################################################################
...
for ii = 1:4
    ...
    fx.cEfflux(ii).maintenance(:,tix)	= fe.RAact.km(ii).value(:,tix) .* s.cPools(ii).value;
    
    % growth respiration: R_g = (1 - YG) * (GPP * allocationToPool - R_m)
    fx.cEfflux(ii).growth(:,tix)	= (1 - p.RAact.YG) .* (fx.gpp(:,tix) .* d.cAlloc.c2pool(ii).value(:,tix) - fx.cEfflux(ii).maintenance(:,tix));
