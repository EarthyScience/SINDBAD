function [fe,fx,d,p] = prec_RAact_Thornley2000C(f,fe,fx,s,d,p,info)
% #########################################################################
...
if p.RAact.flagMTF
    for ii = 1:4 % for all the vegetation pools
        ...
        p.RAact.Fd(ii).value = fe.cCycle.MTF;
    end
else
    for ii = 1:4 % for all the vegetation pools
        p.RAact.Fd(ii).value = 1;
    end
end
...
for ii = 1:4 % for all the vegetation pools
    km                          = 1 ./ p.RAact.C2N(ii).value .* RMN .* d.TempEffectRAact.fT(ii).value;
    kd                          = p.RAact.Fd(ii).value;
    fe.RAact.km(ii).value	= km .* kd;
    fe.RAact.km4su(ii).value	= fe.RAact.km(ii).value .* (1 - p.RAact.YG);
end
