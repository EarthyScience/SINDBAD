...
tc      = p.pveg.TreeCover;
rf2rc	= p.cAlloc.Rf2Rc;

% the allocation fractions according to the partitioning to root/wood/leaf
% - represents plant level allocation
r0	= d.cAlloc.cf2Root(:,tix); % this is to below ground root fine+coarse
s0	= d.cAlloc.cf2Wood(:,tix);
l0	= d.cAlloc.cf2Leaf(:,tix);
...
% feed it to d
d.cAlloc.c2pool(1).value(:,tix)	= rf1;
d.cAlloc.c2pool(2).value(:,tix)  = rc1;
d.cAlloc.c2pool(3).value(:,tix)  = s1;
d.cAlloc.c2pool(4).value(:,tix)	= l1;
