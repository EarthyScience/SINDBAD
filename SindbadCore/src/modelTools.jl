...
"""
    getInOutModel(model::SindbadCore.Types.LandEcosystem)
    getInOutModel(model::SindbadCore.Types.LandEcosystem, model_func::Symbol)
    getInOutModel(model::SindbadCore.Types.LandEcosystem, model_funcs::Tuple)

Parses and retrieves the inputs, outputs, and parameters (I/O/P) of SINDBAD models for specified functions or all functions.

# Arguments:
- `model::SindbadCore.Types.LandEcosystem`: A SINDBAD model instance. If no additional arguments are provided, parses all inputs, outputs, and parameters for all functions of the model.
...
function getInOutModel(T::Type{<:SindbadCore.Types.LandEcosystem}; verbose=false)
    return getInOutModel(T(), verbose=verbose)
end

function getInOutModel(model::SindbadCore.Types.LandEcosystem; verbose=true)
    if verbose
        println("   collecting I/O/P of: $(nameof(typeof(model))).jl")
    end
    mo_in_out=SindbadCore.DataStructures.OrderedDict()
...
function getInOutModel(model::SindbadCore.Types.LandEcosystem, model_funcs::Tuple)
    mo_in_out=SindbadCore.DataStructures.OrderedDict()
    println("   collecting I/O/P of: $(nameof(typeof(model))).jl")
...
function getInOutModel(model, model_func::Symbol)
    ...
    mod_vars = SindbadCore.DataStructures.OrderedDict{Symbol, Any}()
    mod_vars[:approach] = model_name
    if model_func == :compute
        mod_code = @code_string SindbadCore.Models.compute(model, nothing, nothing, nothing)
    elseif model_func == :define
        mod_code = @code_string SindbadCore.Models.define(model, nothing, nothing, nothing)
    ...
    elseif model_func == :precompute
        mod_code = @code_string SindbadCore.Models.precompute(model, nothing, nothing, nothing)
    elseif model_func == :update
        mod_code = @code_string SindbadCore.Models.update(model, nothing, nothing, nothing)
    else
...
function getInOutModels(ind_range=1:10000::UnitRange{Int64})
    ...
    sm_io = SindbadCore.DataStructures.OrderedDict()
    for s in sm_list[s_ind:e_ind]
        s_apr = sind_m_dict[s]
        if !isempty(s_apr)
            s_apr_s = join(s_apr, ".jl, ") * ".jl"
            sm_io[s]=SindbadCore.DataStructures.OrderedDict()
            map(s_apr) do s_a
...
function getInOutModels(models::Tuple)
    mod_vars = SindbadCore.DataStructures.OrderedDict()
...
function getTypedModel(model::Symbol, model_timestep="day", num_type=Float64)
    model_obj = getfield(SindbadCore.Models, model)
...
function getParameterValue(model, parameter_name, model_timestep)
    ...
    p_timescale = SindbadCore.Models.timescale(model, parameter_name)
...
"""
    ...
    modelParameter(model::SindbadCore.Types.LandEcosystem, show=true)
...
- `model::SindbadCore.Types.LandEcosystem`: A SINDBAD model instance of type LandEcosystem.
...
function modelParameter(models, model::Symbol)
    ...
    p_dict = SindbadCore.DataStructures.OrderedDict()
    if length(pnames) == 0
        ...
        foreach(pnames) do fn
            ...
            p_unit = SindbadCore.Models.units(mod, fn)
...
function modelParameter(model::SindbadCore.Types.LandEcosystem, show=true)
    ...
    if length(pnames) == 0
        ...
        p_vec = map(pnames) do fn
            ...
            p_describe = SindbadCore.Models.describe(model, fn)
            p_unit = SindbadCore.Models.units(model, fn)
            p_u = isempty(p_unit) ? "`unitless`" : "units: `$(p_unit)`"
            p_timescale = SindbadCore.Models.timescale(model, fn)
            p_t = isempty(p_timescale) ? "`all` timescales" : "`$(p_timescale)` timescale"
            p_bounds = SindbadCore.Models.bounds(model, fn)
