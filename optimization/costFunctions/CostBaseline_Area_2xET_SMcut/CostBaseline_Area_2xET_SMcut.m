function [cost, costComps] = CostBaseline_Area_2xET_SMcut(f,fe,fx,s,d,p,obs,info)
...
% costSM    = 'robust' correlation (SM), considering only grids with at
% least 60% og months available
...
% SM: only consider grids with 60% of months available
sm_avail     = NaN(size(SMobs));
sm_avail(~isnan(SMobs)) = 1;
sm_cnt    = sum(sm_avail,2, 'omitnan');
sm_pct    = (sm_cnt .* 100) ./ 96; %SM data is only available from 2007 -> only 96 months
xx = 60;
SMobs(sm_pct<xx,:) = NaN;
