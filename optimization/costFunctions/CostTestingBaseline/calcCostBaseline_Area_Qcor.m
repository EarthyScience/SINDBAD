function [cost, costComps] = calcCostBaseline_Area_Qcor(f,fe,fx,s,d,p,obs,info)
...
% costQ     = 'robust' correlation (MSC_Q)
...
%%%%% correlation for MSC of Q 
% q_resid     =   sum( abs(Q_MSCobs(vQ)-Q_MSCmod(vQ)) );
% q_var       =   sum( abs(Q_MSCobs(vQ)-mean(Q_MSCobs(vQ))) );
% costQ       =   q_resid/q_var;
corQ      =   corr(Q_MSCobs(vQ),Q_MSCmod(vQ), 'rows', 'pairwise');
costQ     =   1-sign(corQ ) .* abs(corQ); % just: 1 - corSM ?
