function [cost, costComps] = calcCostCumSum_Area(f,fe,fx,s,d,p,obs,info)
% cumsum cost function using robust metrics, data uncertainty and area
% weights
% tVec      = vector with month (M)
% cost      = costTWS + costET + costQ
% costTWS   = weighted robust MAE(TWS)
% costSWE   = weighted robust MAE(SWE)
% costSM    = 'robust' correlation (SM)
% costET    = weighted MAE of cumsum(MSC_ET)
% costQ     = weighted MAE of cumsum(MSC_ET)
% [cost, costComps]    = calcCostCumSum_Area([],[],fx,[],d,[],obs,info)
...
try
    TWSobs          = obs.TWS.data .* gridArea_m;
...
if isfield(d.storedStates,'wSnow')
    SWEmod_d  = squeeze(d.storedStates.wSnow); % for SWE .* gridArea after monthly aggregation and removing of the threshold
else
...
% calculate MSC of ET
ET_MSCobs        = calcMSC(ETobs,tVec);
ET_MSCmod        = calcMSC(ETmod,tVec);
ET_MSCobs_uncert = calcMSC(ETobs_uncert,tVec);
...
vET     = find(~isnan(ET_MSCobs));
vQ      = find(~isnan(Q_MSCobs));

%%%%% weighted robust MEF for TWS
tws_resid   =   mean( abs(TWSobs(vTWS)-TWSmod(vTWS)) ./ abs(TWSobs_uncert(vTWS)) );
tws_var     =   sum( abs(TWSobs(vTWS)-mean(TWSobs(vTWS))) ./ abs(TWSobs_uncert(vTWS)) );

costTWS     =   tws_resid;

%%%%% weighted robust MEF for SWE
swe_resid   =   mean( abs(SWEobs(vSWE)-SWEmod(vSWE)) ./ abs(SWEobs_uncert(vSWE)) );
...
%%%%% weighted MAE of cumsum for MSC of ET
cumsum_ET_MSCobs    =   cumsum(ET_MSCobs .* 30,2);
cumsum_ET_MSCmod    =   cumsum(ET_MSCmod .* 30,2);
cumsum_ET_MSCuncert =   cumsum(ET_MSCobs_uncert  .* 30,2);

costET      =   mean( abs(cumsum_ET_MSCobs(vET) - cumsum_ET_MSCmod(vET)) ./ abs(cumsum_ET_MSCuncert(vET)) );

%%%%% weighted MAE of cumsum for MSC of Q
cumsum_Q_MSCobs     =   cumsum(Q_MSCobs .* 30,2);
cumsum_Q_MSCmod     =   cumsum(Q_MSCmod .* 30,2);
cumsum_Q_MSCuncert  =   cumsum(Q_MSCobs_uncert .* 30,2);

costQ       =   mean( abs(cumsum_Q_MSCobs(vQ) - cumsum_Q_MSCmod(vQ)) ./ abs(cumsum_Q_MSCuncert(vQ)) );
