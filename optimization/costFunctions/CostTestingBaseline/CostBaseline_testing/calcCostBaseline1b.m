function [cost] = calcCostBaseline1b(f,fe,fx,s,d,p,obs,info)
...
% costET    = wrobustMEF(MSC_ET)
% costSM    = sign(cor(SM))*cor(SM)^2
% costQ     = wrobustMEF(MSC_Q)
% [costTotal,costTWS,costSWE,costET,costSM,costQ]=calcCostBaseline1b(f,fe,fx,s,d,p,obs,info)
...
tmp1    =   mean(abs(ETobs_MSC(v_et2)-ETmod_MSC(v_et2)) ./ abs(ETobs_uncert_MSC(v_et2)));
tmp2    =   mean(abs(ETobs_MSC(v_et2)-mean(ETobs_MSC(v_et2))) ./ abs(ETobs_uncert_MSC(v_et2)));
