...
defopts.SaveVariables = 'off   % [on|final|off][-v6][-v7.3] save variables to .mat file';
...
%Directory to look for CMAES files and save the output
defopts.outDirPath='./out.CMAES/';
%sujan
if isfield(inopts,'outDirPath')
    CMAESDIR=inopts.outDirPath;
    inopts.SaveFilename=[inopts.outDirPath inopts.SaveFilename];
else
...
if isempty(fitfun)
    ...
    % disp([pad('CMAES',20) ' : ' 'Objective function not determined, ''' fitfun ''' used']);
...
while irun <= myeval(opts.Restarts) % for-loop does not work with resume
    ...
    if strfind(opts.SaveVariables, '-v7.3')
        i = strfind(opts.SaveVariables, '%');
        if isempty(i) || i == 0 || strfind(opts.SaveVariables, '-v7.3') < i
            strsaving = '-v7.3';
            flgsaving = 1;
            flgsavingfinal = 1;
        end
    ...
    if flgresume % resume is on
        ...
        if bnd.isactive
            ...
            if any(sigma*sqrt(diagC) > maxdx)
                ...
                disp([pad('CMAES',20) ' : ' 'Initial SIGMA multiplied by the factor ' num2str(fac) ...
            ...
            if any(5*sigma*sqrt(dd(idx)) < ubounds(idx) - lbounds(idx))
                disp([pad('CMAES',20) ' : ' 'Initial SIGMA is, in at least one coordinate, ' ...
        ...
        if irun == 1
            ...
            if savemodulo && savetime
                ...
                for namecell = filenames(:)'
                    ...
                    [fid, err] = fopen([CMAESDIR filenameprefix name '.dat'], 'w');
                    if fid < 1 % err ~= 0
    ...
    allCostTina = [bestever.f ];
    allParaTina = [bestever.x ];
    ...
    while isempty(stopflag)
        ...
        for k=find(isnan(fitness.raw)),
            ...
            while isnan(fitness.raw(k))
                ...
                % Tina: output cost components (in a structure) as well
                [fitness.raw(k), costComp]  = feval(fitfun,arxvalid(:,k)');
                
                % Tina: transform the structure with all cost components in
                % a table and add it as a new line in the allCostComp table
                allCostComp = [allCostComp; struct2table(costComp)];
                ...
                if mod(tries, 100) == 0
                    disp([pad('CMAES',20) ' : ' num2str(tries) ...
          ...
          %% fitness.raw (cost) & arxvalid (param)
          
          allCostTina = [allCostTina fitness.raw];
          allParaTina = [allParaTina arxvalid];
        ...
        if ~flgDiagonalOnly && (ccov1+ccovmu+neg.ccov) > 0 && mod(countiter, 1/(ccov1+ccovmu+neg.ccov)/N/10) < 1
            ...
            if min(diagD) <= 0
                if stopOnWarnings
                    ...
                    disp([pad('CMAES',20) ' : ' 'Iteration ' num2str(countiter) ...
            ...
            if max(diagD) > 1e14*min(diagD)
                if stopOnWarnings
                    ...
                    disp([pad('CMAES',20) ' : ' 'Iteration ' num2str(countiter) ': condition of C ' ...
                        'at upper limit' ]);
        ...
        if any(sigma*sqrt(diagC) > maxdx)
            sigma = min(maxdx ./ sqrt(diagC));
            %disp([pad('CMAES',20) ' : ' 'Iteration ' num2str(countiter) ': coordinate axis std ' ...
        ...
        if savemodulo && savetime && (countiter < 1e2 || ~isempty(stopflag) || ...
            ...
            for namecell = filenames(:)'
                ...
                [fid, err] = fopen([CMAESDIR filenameprefix name '.dat'], 'a');
                if fid < 1 % err ~= 0
                    disp([pad('CMAES',20) ' : ' 'could not open ' filenameprefix name '.dat']);
                else
                    if strcmp(name, 'axlen')
        ...
        if ~isempty(stopflag) || time.saving < 0.05 * time.nonoutput || countiter == 100
            ...
            if flgsaving && countiter > 2
                ...
                if ~isempty(strsaving) && ~isoctave
                    save('-mat', strsaving, opts.SaveFilename, '-v7.3'); % for inspection and possible restart
                else
                    save('-mat', opts.SaveFilename, '-v7.3'); % for inspection and possible restart
                end
...
for name = fieldnames(inopts)'
    ...
    if sum(idx) == 1
        ...
        elseif isstruct(getfield(defopts, defname))
            ...
            disp([pad('CMAES',20) ' : ' 'option "' name '" disregarded (must be struct)']);
        ...
        disp([pad('CMAES',20) ' : ' 'option "' name '" disregarded (unknown field name)']);
    end
end
...
if any(diff(arf1) == 0)
    ...
    disp([pad('CMAES',20) ' : ' num2str(sum(diff(arf1)==0)) ' equal function values']);
end
